...
function connectionListener(socket) {
	...
	In.on('data', function (line) {
		...
		if (socket.state == 'data') {
			...
			var m
			line = line.trimRight()
			if(m = line.match(/^HELO\s+(.*)/i)) {
				var helodata = {name: m[1], valid: true, extended: false }
				socket.emit('helo', helodata)
				if(helodata.valid) {
				...
				socket.emit('message', socket.currentMessage)
