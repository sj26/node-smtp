	...
	stream.on('data', function (data) {
		...
		for(i=0; i<dataArray.length-1; i=i+1) {
			...
			if (stream.state=='data') {
				...
				if(cmd == 'MAIL' && left(dataArray[i], 10).toUpperCase() == 'MAIL FROM:'){stream.out('250 MAIL...I hope thats right :)'); touch=true;}
				if(cmd == 'RCPT' && left(dataArray[i], 8).toUpperCase()	== 'RCPT TO:'	){stream.out('250 RCPT...I hope thats right :)'); touch=true;}
				...
				if(cmd == 'EHLO' || left(data, 4)=='SEND' || left(data, 4)=='SAML' || left(data, 4)=='SOML' | left(data, 4)=='TURN'){stream.out('502 Unsupported here'); touch=true;}
